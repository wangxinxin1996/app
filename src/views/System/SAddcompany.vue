<template>
  <div class="Saddcompany">
      ...
      <el-row>
        ...
        <el-col :span="8">
          <el-form-item>
            ...
            <el-col :span="17">
              <el-input v-model="formInline.userNo" placeholder="企业编码"></el-input>
            </el-col>
          </el-form-item>
        </el-col>
      </el-row>
    ...
    <el-button
      ...
      @click="addcompany()"
    >添加企业</el-button>
    <el-button type="primary" size="small" icon="search" @click="onMoreCreateUser()">excel导入</el-button>
    <el-table
      :data="Usermessage"
      style="width: 100%"
      ref="multipleTable"
    ...
    >
      ...
      <el-table-column prop="countName" label="企业编码">
      ...
      <el-table-column prop="password" label="企业类型">
        <template slot-scope="scope">
          <span>{{ scope.row.password }}</span>
        </template>
      </el-table-column>
      <el-table-column prop="identity" label="企业地址">
        <template slot-scope="scope">
          <span>{{ scope.row.identity }}</span>
        </template>
      </el-table-column>
      <el-table-column prop="identity" label="负责人">
        <template slot-scope="scope">
          <span>{{ scope.row.identity }}</span>
        </template>
      </el-table-column>
      <el-table-column prop="identity" label="负责人电话">
        <template slot-scope="scope">
          <span>{{ scope.row.identity }}</span>
        </template>
      </el-table-column>
      <el-table-column fixed="right" label="操作" width="120">
        <template slot-scope="scope">
          <el-button
            @click.native.prevent="deleteRow(scope.$index, Usermessage)"
            type="text"
            size="small"
          >移除</el-button>
        </template>
